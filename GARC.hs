...
getWord16le' = fromIntegral <$> getWord16le
getWord32le' = fromIntegral <$> getWord32le
...
instance Binary GARC where
    ...
    get = do
        ...
        GARC <$> getFIMB dataOffset fileOffsets

-- GARC header
getGARCHeader :: Get Int
getGARCHeader = do
    "CRAG" <- BLC.unpack <$> getLazyByteString 4  -- "Magic bytes"
...
getFATO :: Get [Int]
getFATO = do
    "OTAF" <- BLC.unpack <$> getLazyByteString 4  -- "Magic bytes"
...
getFATB offsets = do
    "BTAF" <- BLC.unpack <$> getLazyByteString 4  -- "Magic bytes"
...
getFIMB filesStart fileOffsets = do
    "BMIF" <- BLC.unpack <$> getLazyByteString 4  -- "Magic bytes"
